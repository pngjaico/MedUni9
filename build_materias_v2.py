...
ARQUIVO_ESTRUTURAS = os.path.join(BASE_DIR, 'conteudos', '_para_categorizar', 'Planos de Ensino', 'planos_estruturados.json')
ARQUIVO_SAIDA = os.path.join(BASE_DIR, 'data', 'materias.json')
DIR_CONTEUDOS = os.path.join(BASE_DIR, 'conteudos', 'materiais')

print(f"📂 Base: {BASE_DIR}")
print(f"📖 Entrada: {ARQUIVO_ESTRUTURAS}")
...
def ajustar_modulo(modulo):
    """Ajusta módulo, excluindo 5 - reatribui para 6."""
    if modulo == 5:
        return 6
    if modulo < 1:
        return 1
...
def inferir_dados_aula(nome_arquivo):
    """Inferir sigla e título baseado no nome do arquivo PDF."""
...
def build():
    """Constrói o banco de dados de matérias."""
    try:
        with open(ARQUIVO_ESTRUTURAS, 'r', encoding='utf-8') as f:
            estruturas = json.load(f)
    except FileNotFoundError:
        print(f"❌ Erro: Arquivo não encontrado: {ARQUIVO_ESTRUTURAS}")
        return
    ...
    for mod, planos in estruturas.items():
        ...
        for nome_pdf, data in planos.items():
            ...
            print(f"✅ {sigla:12} → Módulo {num_modulo} | {len(aulas_db):2}+ aulas | {titulo}")

    # Escreve o banco final
