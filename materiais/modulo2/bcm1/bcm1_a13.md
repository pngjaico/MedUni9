# BCM1 — Aula 13: Genética Médica — Heredograma e Herança

A genética médica estuda como as doenças passam de pais para filhos. Na prova, o tema central é a leitura de **Heredogramas** (árvores genealógicas) e entender as regras de Mendel: **Autossômica Dominante**, **Recessiva**, **Ligada ao X** e a herança **Mitocondrial**, além de exceções como o **Imprinting**.

Na prática, saber interpretar um heredograma permite ao médico calcular o risco de um casal ter um filho com uma doença grave e orientar sobre a necessidade de exames genéticos.

---

### Figura sugerida

**Figura-ID:** `BCM1-A13-F01`

- **Momento:** Seção de padrões de herança.
- **O que mostrar:** O "Caminho da Herança": Quatro exemplos de heredogramas lado a lado. 1) Dominante (Vertical, todas as gerações); 2) Recessiva (Horizontal, pais normais com filho doente); 3) Ligada ao X (Só homens doentes); 4) Mitocondrial (Mãe doente passa para todos os filhos).
- **Tipo sugerido:** Infográfico didático.
- **Legenda (rascunho):** Como identificar os padrões de herança em uma árvore genealógica.

## 1. Herança Autossômica Dominante (AD)

Basta **um alelo** (versão do gene) mutado para ter a doença.
- **Características:** Transmissão vertical (não pula gerações). Afeta homens e mulheres igualmente.
- **Chance:** Se um pai é afetado, o filho tem **50% de chance** de herdar.
- **Exemplos:** Acondroplasia (Nanismo), Doença de Huntington e Neurofibromatose.

---

## 2. Herança Autossômica Recessiva (AR)

Precisa de **dois alelos** mutados (um do pai e um da mãe) para ter a doença.
- **Características:** Transmissão horizontal (pula gerações). Pais costumam ser saudáveis (portadores).
- **Chance:** Se os dois pais são portadores, o filho tem **25% de chance** de ser doente e 50% de ser portador saudável.
- **Exemplos:** Fibrose Cística e Anemia Falciforme. A **Consanguinidade** (pais parentes) aumenta muito o risco aqui.

---

## 3. Herança Ligada ao X (XLR)

O erro está no cromossomo X.
- **Características:** **Afeta quase exclusivamente homens**. A mulher costuma ser apenas portadora.
- **Regra de Ouro:** **Não existe transmissão de pai para filho homem** (o pai dá o Y para o filho, não o X).
- **Exemplos:** Hemofilia e Distrofia de Duchenne.

---

## 4. Herança Mitocondrial e Imprinting

- **Mitocondrial:** Só a **MÃE** transmite. Se o pai for doente e a mãe saudável, 100% dos filhos serão saudáveis.
- **Imprinting (Cromossomo 15):** A doença depende de qual progenitor mandou o gene estragado:
  - Se faltar o gene do **PAI** → Síndrome de **Prader-Willi** (Obesidade e fome excessiva).
  - Se faltar o gene da **MÃE** → Síndrome de **Angelman** (Riso fácil e atraso grave).

---

## Ponte com a Clínica

O diagnóstico de **Distrofia Muscular de Duchenne** é um clássico da herança ligada ao X. Um menino que começa a cair muito e tem as panturrilhas muito grandes (pseudohipertrofia) deve ser investigado. Como a herança é ligada ao X, o médico deve olhar para a família da mãe (tios maternos ou avô materno) para ver se há outros casos. Isso ajuda a confirmar o padrão de herança antes mesmo do teste genético chegar.

---

## Pontos-Chave para Prova

- **Dominante**: Vertical, 50% de risco.
- **Recessiva**: Horizontal, 25% de risco, pais saudáveis.
- **Ligada ao X**: Homens afetados, mãe portadora, **sem transmissão pai-filho**.
- **Mitocondrial**: Exclusivamente **Materna**.
- **Penetrância Incompleta**: Você tem o gene, mas a doença não se manifestou (parece que pulou geração, mas é dominante).
- **Antecipação**: A doença fica mais grave e começa mais cedo a cada geração (Ex: Huntington).

---

## Pré-Prova

<ctrl94> Leia isso 30 minutos antes da prova. Vai direto ao ponto.

### Síntese para a prova

Para gabaritar Genética, foque no heredograma. 
Se você ver a doença em todas as gerações, marque **Autossômica Dominante**. Se os pais são saudáveis e o filho é doente, marque **Autossômica Recessiva** (e procure por casamentos entre primos). Se só houver homens doentes e o pai doente tiver apenas filhos homens saudáveis, é **Ligada ao X Recessiva**.

A Uninove adora o **Imprinting do Cromossomo 15**. Decore o macete: **P**ai ausente = **P**rader-Willi. **M**ãe ausente = Angel**M**an. Sobre a herança mitocondrial, lembre-se que o espermatozoide não doa mitocôndrias, por isso o pai nunca transmite esse tipo de doença. E se a questão falar em um gene causando problemas no coração, no olho e no osso ao mesmo tempo (como na Síndrome de Marfan), o nome disso é **Pleiotropia**.

### Diferenciações que a Uninove adora cobrar

| Conceito A | Conceito B | Diferencial Crítico |
|------------|------------|-----------------|
| **Dominante** | **Recessiva** | Vertical (50%) vs **Horizontal (25% + Pais sadios)** |
| **X-ligada** | **Mitocondrial** | Passa pela mãe, mas pai afeta filhas vs **Passa EXCLUSIVAMENTE pela mãe** |
| **Prader-Willi** | **Angelman** | Falta gene do Pai (Comilão) vs **Falta gene da Mãe (Risonho)** |
| **Penetrância** | **Expressividade** | Ter ou não a doença vs **Quão grave é a doença** |
| **Pai Afetado (XLR)** | **Mãe Portadora (XLR)** | 100% filhas portadoras vs **50% filhos doentes** |

### Frase-âncora para não esquecer

> "Dominante não pula, Recessiva é horizontal; se no X o pai não passa pro filho, é herança sexual. Mãe comanda a mitocôndria e o 15 é o canal: sem o pai é Prader-Willi, sem a mãe Angelman é o sinal."
