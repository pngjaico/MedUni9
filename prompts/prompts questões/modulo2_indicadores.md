# Refatorar questões — Indicadores de Saúde: Epidemiologia e Bioestatística (`indicadores`)

**Módulo:** 2 · **Sigla:** IND · **Ficheiro de saída:** `data/questoes.json`
...
3. Para cada aula, **antes** de reescrever as 10 questões, leia o Markdown em:
   - `data/materiais/indicadores/<aula_id>.md`
   - se faltar, espelho: `materiais/modulo2/indicadores/<aula_id>.md`

## Escopo desta disciplina

- `materia` (JSON): `indicadores`
- Aulas (ordem do catálogo — cada `aula_id` tem **10** questões):  
  ind_a1, ind_a2, ind_a3, ind_a4, ind_a5, ind_a6, ind_a7, ind_a8, ind_a9, ind_a10, ind_a11
...
1. Ler o `.md` da aula.
2. Isolar em `data/questoes.json` as **10** entradas com `"materia": "indicadores"` e `"tema": "<aula_id>"`.
