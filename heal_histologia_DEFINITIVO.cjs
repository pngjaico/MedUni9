...
const fragments = {
  ...
  "TERRITO": "RIAL", "FORMAN": "DO", "GRUPOS": "ISOGENOS", "GRUPOSISOGENOS": "AXIAIS",
  "GRUPOS": "ISOGENOS", "PROFA": "SE", "META": "SE", "ANA": "FASE", "TELO": "FASE"
};
...
function healLegenda(lines, slideTitle) {
  ...
  return merged.map(s => {
    ...
    // Remove trailing dots that are not necessary
    res = res.replace(/\.+$/, '');
    return res;
...
console.log(`Sucesso DEFINITIVO!`);
