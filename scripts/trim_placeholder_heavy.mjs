/**
 * trim_placeholder_heavy.mjs
 * Demotes placeholder essenciais in bioe, semio4, bmf4, ff4.
 * Placeholder = enunciado starts with "[QUESTÃO ESSENCIAL ...]"
 * Real questions kept; artifacts in kept questions fixed.
 *
 * bioe  (79 demotions): a2-a12 mostly placeholders; a1+a9=real only
 * semio4 (78 demotions): a1=1 real + 7 ph; a2-a10=all ph
 * bmf4  (138 demotions): a1,a4,a7,a10,a13,a16=1 real each + 7 ph; rest all ph
 * ff4   (110 demotions): a1,a9=1 real each + 7 ph; rest all ph
 */
...
const REBAIXAR = new Set([
  ...
  // semio4 placeholders (78): a1 keeps 5025; a7 keeps 5080
  ...
  // bmf4 placeholders (138): keeps 4769,4793,4817,4841,4865,4889
  ...
  // ff4 placeholders (110): keeps 4913, 4977
...
// Fix trailing ), artifacts in KEPT real questions
const FIX_TRAILING = new Set([
  // bioe kept real questions
  4673, 4674, 4675, 4676, 4677, 4678, 4679, 4680,
  4705, 4713, 4721, 4729, 4753,
  // semio4 kept
  5025, 5080,
  // bmf4 kept
  4769, 4793, 4817, 4841, 4865, 4889,
  // ff4 kept
  4913, 4977,
]);
...
// Audit per materia
for (const mat of ['bioe','semio4','bmf4','ff4']) {
  const byAula = {};
  for (const q of questoes) {
