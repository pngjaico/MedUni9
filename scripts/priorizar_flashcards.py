...
Gera fila de prioridade para o agente gerador de flashcards.

Saída: data/agent_logs/status_flashcards.json

Uso:
  python scripts/priorizar_flashcards.py
  python scripts/priorizar_flashcards.py --modulo 2
  python scripts/priorizar_flashcards.py --materia bcm1
  python scripts/priorizar_flashcards.py --meta 8   (muda meta para 8 flashcards)
"""
...
STATUS_PATH = ROOT / "data" / "agent_logs" / "status_flashcards.json"
...
def main():
    parser = argparse.ArgumentParser(description="Prioriza aulas para geração de flashcards.")
    parser.add_argument("--modulo", type=int, default=None)
    parser.add_argument("--materia", type=str, default=None)
    parser.add_argument("--meta", type=int, default=META_FLASHCARDS)
    ...
    fila = []
    stats = {"total_aulas_com_material": 0, "com_meta_atingida": 0, "sem_nenhum": 0, "total_flashcards": len(flashcards)}
    ...
    for materia_id, info in materias.items():
        ...
        for aula in aulas:
            ...
            # Score: aulas sem nada primeiro, depois as que faltam mais
            score = faltam * 10 + (100 if fc_count == 0 else 0)
            # Módulos iniciais recebem prioridade
            if modulo in (1, 2):
                score += 15
            ...
            fila.append({
                ...
                "flashcards_atuais": fc_count,
                "flashcards_faltam": faltam,
    ...
    status = {
        ...
        "meta_flashcards_por_aula": meta,
        "stats": stats,
        "total_na_fila": len(fila),
        "fila_prioridade": fila[:60],
        "acoes": [
            {"tipo": "info", "hora": datetime.now().strftime("%H:%M"),
             "texto": f"{len(fila)} aulas na fila | {stats['sem_nenhum']} sem nenhum flashcard"}
        ],
    }
    ...
    print(f"Priorização de flashcards — {datetime.now().strftime('%d/%m/%Y %H:%M')}")
    print(f"  Aulas com material: {stats['total_aulas_com_material']}")
    print(f"  Com meta atingida ({meta} cards): {stats['com_meta_atingida']}")
    print(f"  Sem nenhum flashcard: {stats['sem_nenhum']}")
    print(f"  Na fila para gerar: {len(fila)}")
    print(f"\nTop 10 urgentes:")
    for item in fila[:10]:
        print(f"  [{item['score']:3d}] {item['aula_id']} — {item['flashcards_atuais']}/{meta} cards — {item['aula_tema'][:40]}")
