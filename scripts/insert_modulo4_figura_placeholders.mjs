/**
 * Insere ### Figura sugerida após ## Relevância Clínica e Acadêmica (Módulo 4).
 */
import fs from "fs";
import path from "path";

const ROOT = path.resolve("materiais/modulo4");
const OUT_JSON = path.resolve("data/new_figuras_modulo4_entries.json");

const CANONICAL = new Set(["bmf4", "semiologia4", "fisiopato_farmaco", "bioestatistica"]);
...
function jsonEntry(disciplina, aulaFile, id, titleLine) {
  ...
  return {
    ...
    disciplina,
    aula,
    caminhoMaterial: `materiais/modulo4/${disciplina}/${aula}.md`,
    ...
    notas: "Placeholder Módulo 4 — preencher URL no Commons quando o módulo estiver revisado.",
  };
}
...
for (const dir of fs.readdirSync(ROOT, { withFileTypes: true })) {
  ...
  if (!CANONICAL.has(dir.name)) {
    report.push({ folder: dir.name, action: "skip (não canónico)" });
    continue;
  }
