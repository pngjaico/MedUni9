/**
 * Reinsere ### Figura sugerida após relevância (Módulo 4).
 */
import fs from "fs";
...
for (const dir of fs.readdirSync(ROOT, { withFileTypes: true })) {
  if (!dir.isDirectory() || !CANONICAL.has(dir.name)) continue;
  ...
  for (const f of fs.readdirSync(sub)) {
    ...
    fs.writeFileSync(full, r.content.replace(/\r\n/g, "\n"), { encoding: "utf8" });
    report.push({ file: rel, action: "repaired", id });
  }
}

const outPath = path.resolve("data/repair_modulo4_figura_report.json");
